...
**Gate visuale aperto:** il vecchio pacchetto contiene 98 asset per piattaforma derivati da soli 18 master e quindi mostra ripetizioni. `scripts/build-case-study-bowling-package.mjs` ora rifiuta il build completo finche non sono disponibili 98 sorgenti uniche, una per scena/slide/post. `--plan-only` aggiorna il piano senza cancellare il pacchetto esistente. Non dichiarare rigenerate le immagini finche questo gate non e stato soddisfatto.
