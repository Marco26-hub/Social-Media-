import { GIORNI_PROVA, SALA_NOTA_PREZZI_EN } from '@/lib/ristoranti-listino'
...
const ELENCO: Settore[] = [
  ...
  },
  {
    ...
    nome: 'Restaurants, pizzerias and bars',
    sommario: 'QR ordering, table payments, bookings, counter collection numbers and a dining room that is easier to control.',
    titoloSeo: 'QR ordering and table payments for restaurants and pizzerias | SWA',
    descrizioneSeo:
      'Customers scan a table QR code, order and pay from their phone. For restaurants, pizzerias and bars in Italy: bookings, menus, live orders, websites and social content.',
    eyebrow: 'Restaurants, pizzerias and bars',
    ...
    servizio: 'QR ordering and table payment system for restaurants and pizzerias',
    tipoServizio: 'QR ordering, table payments, bookings, website and content for restaurants, pizzerias and bars',
    promessa:
      'Orders and payments move through the customer phone, bookings enter one dashboard, and we take no percentage of what you take. No promise on cover numbers.',
    notaPrezzi: SALA_NOTA_PREZZI_EN,
    segnali: ['No percentage taken on your takings', 'QR ordering and payment at the table', 'Your payment provider stays yours'],
    risultati: [
      ...
      { title: 'Your takings stay whole', text: 'Card payments land in the venue account and we keep no percentage. A low fee with one or two per cent on top means someone else collects first and passes you the rest.' },
    ],
    cosaTitolo: 'What we install in the room, and around it.',
    cosaIntro:
      'The flow covers the customer journey: book, sit down, order, pay, leave a review and request an invoice. The website and content bring people to that room.',
    cosaFacciamo: [
      { title: 'Table QR', text: 'Each table has its own code. The customer opens the menu in the browser, without installing an app.' },
      { title: 'Phone payment', text: 'Card, Apple Pay, Google Pay or Satispay. The money lands in the venue account and we keep no percentage of it.' },
      { title: 'Split the bill by dish', text: 'Each guest pays for their own dishes from their own phone. The bill closes by itself once everything is settled.' },
      { title: 'Allergens as required', text: 'A dedicated field on every dish, as EU Reg. 1169/2011 requires. The guest reads them without asking staff.' },
      { title: 'A menu you edit yourself', text: 'Dishes, prices and availability update from the panel: the sea bass runs out, you remove it, and it disappears from every table at once.' },
      { title: 'Bookings in one place', text: 'A booking page for your site and socials, capacity checks, a reminder the day before and a cancellation link, so a forgotten table frees up in time.' },
      { title: 'Reviews asked at the table', text: 'Right after the meal, phone already in hand. Whoever is unhappy tells you tonight, instead of telling Google next week.' },
      { title: 'Counter collection numbers', text: 'For takeaway counters: each order gets a number that restarts at one every day, and the counter screen shows what to call.' },
      { title: 'Fixed-price formats', text: 'All you can eat with a per-person price, lunch and dinner separated, and ordering in waves — without it, a table of six sends eighty dishes in three minutes.' },
      { title: 'Electronic invoicing', text: 'The customer enters the details from the table and the invoice goes to the SDI through an accredited intermediary you connect.' },
      { title: 'Your brand, never ours', text: 'Logo, colours and venue details on every page the customer sees. Our name appears nowhere.' },
      { title: 'A menu Google and AI can read', text: 'The public menu page carries structured data: it is what search engines and assistants read when somebody asks where to eat.' },
      { title: 'Separate access by role', text: 'Owner, manager, floor and kitchen each see their own. Floor staff never touch takings or fiscal data.' },
      { title: 'Menu imported, not retyped', text: 'From a CSV or TSV file, or from a Tilby till. Nobody retypes two hundred dishes on day one.' },
    ...
    ciclo: [
      ...
      { number: '02', title: 'Configuration', text: 'We load the menu from a file or the till, generate the QR codes, connect your own payment account and put your brand on the customer pages.' },
      { number: '03', title: 'Room test', text: `We start with a small number of tables and adjust the flow with staff present. The trial runs ${GIORNI_PROVA} days and asks for no card.` },
      { number: '04', title: 'Operations', text: 'The dashboard shows orders, bookings, payments and menu updates.' },
    ],
    faq: [
      { q: 'What does the room system cost?', a: SALA_NOTA_PREZZI_EN },
      { q: 'Do you take a percentage of what we take?', a: 'No. The money never passes through us: card payments land in the venue account from the first moment, and card fees are paid to your own provider. A low monthly fee with a single percentage on top means someone else collects and passes you the rest — at low volume it can be cheaper, but you lose the direct relationship: you cannot negotiate the rate, and you cannot change provider without changing system.' },
      { q: 'Do we have to replace the card machine we already have?', a: 'No. If you have a terminal and an acquirer on a good rate, keep them: the bill closes by recording the payment on your terminal and the table frees up all the same.' },
      { q: 'Does the customer need to install an app?', a: 'No. The QR opens a web page in the phone browser: menu, order and payment all live there. Nothing to install and no account to create in order to eat.' },
      { q: 'Can we buy only the bookings?', a: 'Yes. The two modules are sold separately: bookings alone work without the room system, for a venue that fills by phone but wants a page to be booked from on its site and socials.' },
      { q: 'What do we need to start?', a: 'An account in the venue name to collect payments, the fiscal details, the menu — from a file or typed into the panel — and a print run of the QR codes, one per table. Electronic invoicing additionally needs an accredited SDI intermediary; the Tilby till connection needs their developer programme.' },
      { q: 'Does your name appear on the customer pages?', a: 'No. Logo, colours and details are the venue’s on every page the customer sees: menu, bill, booking and review. Our name appears nowhere.' },
      ...
      { q: 'Can we try it first?', a: `Yes, ${GIORNI_PROVA} days and with no card. You load your own menu, print the QR codes and let one real table use it: you judge it on your dishes, not on a demo.` },
    ],
    correlati: [
