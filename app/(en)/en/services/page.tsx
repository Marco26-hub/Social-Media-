  ...
  return (
    <main id="main-content" className={styles.main}>
      ...
      <section className={styles.section}>
        ...
        <div className={styles.grid}>{services.map(service => { const Icon = service.icon; return <article id={service.id} className={styles.service} key={service.id}><Icon size={25} aria-hidden="true" /><h3>{service.title}</h3><p>{service.text}</p><Link href={service.href ?? '/en/pricing'}>{service.href ? 'Read the full page' : 'Scope and pricing'} <ArrowRight size={15} aria-hidden="true" /></Link></article> })}</div>
      </section>
