...
export async function POST(request: Request) {
  try {
    await requireAuth()
    const { cliente_id, platform, brand, product, obiettivo, budget, model, openrouter_key, gemini_key, opencode_key, agnes_key, quality, quality_level, post_quality, qualita } = await request.json()
    if (!platform) {
