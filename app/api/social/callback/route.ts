...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!igAccounts.length) {
      return NextResponse.redirect(`${base}/dashboard/marketing?tab=performance&connect=no_ig`)
    }
    ...
    return NextResponse.redirect(`${base}/dashboard/marketing?tab=performance&connect=ok&accounts=${igAccounts.length}`)
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'errore'
    return NextResponse.redirect(`${base}/dashboard/marketing?tab=performance&connect=error&msg=${encodeURIComponent(msg.slice(0, 120))}`)
  }
}
