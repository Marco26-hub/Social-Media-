...
const reels = [
  { id: 1, folder: 'REEL_01_REGIA_NON_FOTO', hook: ['IL TUO BOWLING NON HA', 'UN PROBLEMA DI FOTO.'], cta: 'SEGUI IL CASO STUDIO', scenes: ['Hai gia momenti che meritano attenzione.', 'Pubblicati a caso restano file isolati.', 'SWA assegna un ruolo a ogni contenuto.', 'Il risultato e una storia riconoscibile.'] },
  { id: 2, folder: 'REEL_02_SERATA_UN_MESE', hook: ['IL TUO BOWLING HA GIA LE STORIE.', 'SWA LE TRASFORMA IN UN PIANO.'], cta: 'SCRIVI BOWLING', scenes: ['Partiamo dai momenti reali.', 'Scegliamo gli angoli che interessano.', 'Diamo a ogni formato un obiettivo.', 'Costruiamo una sequenza che porta avanti.'] },
  { id: 3, folder: 'REEL_03_PUBBLICHI_QUANDO_CAPITA', hook: ['PUBBLICARE A CASO', 'COSTA PIU DI QUANTO PENSI.'], cta: 'CONTROLLA IL CALENDARIO', scenes: ['Ogni post parte da zero.', 'Il pubblico non riconosce una promessa.', 'I dati non spiegano cosa migliorare.', 'Una regia rende ogni uscita misurabile.'] },
  ...
  { id: 7, folder: 'REEL_07_PISTA_5_CONTENUTI', hook: ['UNA SOLA SERATA.', 'CINQUE ANGOLI EDITORIALI.'], cta: 'SCOPRI LA MAPPA SWA', scenes: ['Atmosfera: fa desiderare lesperienza.', 'Gesto: trattiene lattenzione.', 'Persone: costruiscono fiducia.', 'Offerta: arriva dopo aver creato valore.'] },
  { id: 8, folder: 'REEL_08_CALENDARIO_INVISIBILE', hook: ['QUELLO CHE IL CLIENTE VEDE', 'PARTE DA CIO CHE NON VEDE.'], cta: 'SALVA IL PROCESSO', scenes: ['Brief: decidiamo lobiettivo.', 'Produzione: creiamo asset coerenti.', 'Approvazione: controlli prima delluscita.', 'Pubblicazione: ogni contenuto ha il suo posto.'] },
  ...
  { id: 17, folder: 'REEL_17_HAI_GIA_FOTO', hook: ['HAI GIA LE FOTO?', 'ALLORA NON RIPARTIRE DA ZERO.'], cta: 'SCRIVI BOWLING', scenes: ['Facciamo audit del materiale.', 'Scartiamo cio che non sostiene il messaggio.', 'Completiamo solo gli scatti mancanti.', 'SWA trasforma larchivio in una regia.'] },
  ...
  { id: 22, folder: 'REEL_22_STORIE_SISTEMA', hook: ['IL TUO BOWLING HA GIA LE STORIE.', 'SWA COSTRUISCE IL SISTEMA.'], cta: 'SCRIVI BOWLING', scenes: ['Audit di cio che hai gia.', 'Piano di cio che serve davvero.', 'Produzione e approvazione ordinate.', 'Pubblicazione e lettura dei risultati.'] },
];
...
const posts = [
  ...
  { id: 18, folder: 'POST_18_CONTENUTO_E_SCELTA', lines: ['IL CONTENUTO NON E IL FILE.', 'E LA DECISIONE', 'CHE VIENE PRIMA.'], cta: 'SCOPRI IL METODO SWA' },
  { id: 24, folder: 'POST_24_STORIE_E_SISTEMA', lines: ['IL TUO BOWLING HA GIA LE STORIE.', 'SWA COSTRUISCE', 'IL SISTEMA.'], cta: 'SCRIVI BOWLING' },
];
...
function svgOverlay({ width, height, label, lines, cta, scene = false }) {
  ...
  const lineY = vertical ? 390 * scale : scene ? 300 * scale : 285 * scale;
  ...
  return Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}">
    ...
    <image href="${logo}" x="${58 * scale}" y="${(vertical ? 165 : 45) * scale}" width="${logoW}" height="${logoH}" preserveAspectRatio="xMidYMid meet"/>
    <text x="${58 * scale}" y="${(vertical ? 270 : 145) * scale}" fill="#d6a839" font-family="Arial, sans-serif" font-size="${24 * scale}" font-weight="700" letter-spacing="${3 * scale}">${esc(label)}</text>
    <rect x="${58 * scale}" y="${(vertical ? 295 : 170) * scale}" width="${70 * scale}" height="${5 * scale}" fill="#a8532d"/>
    ${text}
...
if (sourceFiles.length < requiredVisualMasters) {
  throw new Error(`QA VISUAL bloccato: il piano richiede ${requiredVisualMasters} immagini sorgente uniche, ma ne esistono ${sourceFiles.length}. Genera prima i visual dal PIANO-EDITORIALE.json; il builder non ricicla fotografie tra contenuti.`);
}
